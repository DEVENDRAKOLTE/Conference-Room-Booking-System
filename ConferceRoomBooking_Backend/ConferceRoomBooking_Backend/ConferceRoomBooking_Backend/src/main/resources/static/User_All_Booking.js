document.addEventListener('DOMContentLoaded', function() {
  ...
  async function fetchBookingsFromBackend() {
    ...
    console.log(userId); 
    try {
      const response = await fetch(`http://192.168.1.10:8080/room/bookings/${userId}`); // Replace with your actual API endpoint
  ...
  function handleCancel(bookingId) {
    ...
    fetch(`http://192.168.1.10:8080/room/bookings/${bookingId}`, {
      method: 'DELETE',
    })
      .then(response => {
        if (!response.ok) {
